...
*/
   ...
   public class VueCourbeEyeDiagramm  extends Vue {
      ...
      public  VueCourbeEyeDiagramm (float [] valeurs, String nom, int nEch) {   
         ...
         for (int i = 0, j=0; i < valeurs.length; i++) {        	 
        	 if(j>=nEch){
           	  j=0;
             }
        	 if (valeurs[i] > yMax)
             ...
             if (valeurs[i] < yMin)
                yMin = valeurs[i];
             coordonnees[i] = new Point2D.Float(j, valeurs[i]);      
    ...
    */
      ...
      public void paint(Graphics g) {
         if (g == null) {
      	...
      	// effacement total
         //Color couleur = new Color(240,40,40,20); //Dernier paramètre = alpha
         ...
         for (int i=1,j=1; i < coordonnees.length; i++,j++) {
